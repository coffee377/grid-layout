...
import { ContainerData, WithFalse } from 'layout-data';
import { mixinEnhance } from 'utils/merge';
...
const Container: React.FC<ContainerProps> = props => {
  ...
  const styles: CSSProperties = stylePreferred
    ? mixinEnhance(true, configProps, data, style) // 组件属性 < 配置文件 < 内联样式
    : mixinEnhance(true, style, configProps, data); // 内联样式 < 组件属性 < 配置文件
