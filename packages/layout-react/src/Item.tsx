...
import { ItemData, WithFalse, CSSProperties } from 'layout-data';
import { calcGridArea } from 'layout-core/helper';
...
const Item: React.FC<ItemProps> = props => {
   ...
   * 属性合并
   */
  const itemProps: ItemProps = mixinEnhance(true, configProps, data);

  /**
   * @description 计算获取单元格区域
   */
  const getGridArea = () => {
    const { gridArea, rowStart, columnStart, rowSpan, columnSpan } = itemProps;
    if (gridArea) {
      return gridArea;
    }
  ...
  const { className, index, style } = itemProps;
  return (
    <div
      className={classNames({ [styles.center]: centralized }, className)}
      style={{
        zIndex: baseZIndex + (index || 0),
        ...style,
